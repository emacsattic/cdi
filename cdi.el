...
;; Version: $Id: cdi.el,v 1.93 2005/01/19 15:47:10 mphodges-guest Exp $
...
(defgroup cdi nil
  ...
  :group 'tools
  :link '(url-link "http://mph-emacs-pkgs.alioth.debian.org/CdiEl.html"))
...
(defcustom cdi-mode-line-image t
  "*If non-nil, add an image to the mode line.
If t, a default image is used; if a string, this should be the
name of an image file.

For non-nil values, this only has an effect if images can be
displayed, and `create-image' can be called successfully."
  :group 'cdi
  :type '(choice (const :tag "Display default image" t)
                 (file  :tag "Display image from file")
                 (const :tag "Don't display any image" nil))
  :set (lambda (sym val)
         (set-default sym val)
         (when (fboundp 'cdi-update-modeline)
           (mapcar (lambda (b)
                     (with-current-buffer b
                       (when (eq major-mode 'cdi-mode)
                         (cdi-update-modeline))))
                   (buffer-list)))))
...
(defun cdi-update-modeline ()
  ...
  (when (and cdi-mode-line-image
    ...
    (condition-case nil
        (let (image)
          (cond
           ((eq cdi-mode-line-image t)
            (setq image
                  ;; Data from the Debian wm-icons package
                  (create-image
                   "/* XPM */
...
"
                   ...
                   t
                   :ascent 'center)))
           ((and (stringp cdi-mode-line-image)
                 (file-readable-p cdi-mode-line-image))
            (setq image
                  (create-image cdi-mode-line-image
                                nil
                                nil
                                :ascent 'center))))
          (when image
